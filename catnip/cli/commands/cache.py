...
def _format_volume(stats: dict) -> str:
    """Format volume from bytes with readable units.
    ...
    if volume_bytes == 0:
        return "0.00 MB"
    if volume_mb >= 0.01:
        return f"{volume_mb:.2f} MB"
    if volume_bytes >= 1024:
        return f"{volume_bytes / 1024:.2f} KB"
    return f"{volume_bytes} bytes"
...
def cache_stats(backend):
    ...
    if backend == "disk":
        ...
        click.echo(f"Volume:         {_format_volume(stats)} ({stats['volume_bytes']} bytes)")
...
def cache_prune(dry_run):
    ...
    if dry_run:
        ...
        click.echo(f"Current volume:  {_format_volume(stats_before)}")
        ...
        click.echo(f"Before:         {stats_before['size']} entries, {_format_volume(stats_before)}")
