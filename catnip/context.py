...
from catnip._rs import CatnipMeta, Scope
...
class _ImportWrapper:
    ...
    def __call__(self, spec, wild=False):
        # Resolve relative paths against the calling file's directory
        caller_dir = None
        meta = self.ctx.globals.get('META')
        if meta is not None:
            try:
                caller_dir = Path(meta.path).parent
            except AttributeError:
                pass
        namespace = self._get_loader().import_module(spec, caller_dir=caller_dir)
        if wild:
...
class Context:
    ...
    def __init__(self, globals=None, locals=None, logger=None, memoization=None):
        ...
        # Module access policy (None = no restriction)
        ...
        # META is always available, even with custom globals.
        self.globals.setdefault('META', CatnipMeta())
    ...
    def pop_scope(self):
        ...
        if self.locals.depth() > 1:
            ...
            raise CatnipWeirdError("Cannot pop the global scope.")
