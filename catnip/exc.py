...
class CatnipError(Exception):
    ...
    def __init__(
        ...
        *,
        ...
        self.start_byte = start_byte  # Raw byte offset for deferred enrichment
...
class CatnipWeirdError(CatnipError):
    """
    Internal interpreter error -- something that should never happen.

    Indicates a bug in Catnip, not a user error.
    Should be reported as a bug.
    """

    def __init__(self, message, *, cause=None, details=None, **kwargs):
        self.cause = cause
        self.details = details or {}
        super().__init__(message, **kwargs)
        try:
            from catnip.weird_log import log_weird_error

            log_weird_error(self)
        except Exception:
            pass


# Backward-compat alias
CatnipInternalError = CatnipWeirdError
