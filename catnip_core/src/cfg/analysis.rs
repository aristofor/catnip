// FILE: catnip_core/src/cfg/analysis.rs
...
fn compute_immediate_dominators(cfg: &mut ControlFlowGraph) {
    ...
    // Reset previous results so recomputing on a mutated CFG (e.g. after LICM
    // inserts guard blocks) cannot leave stale dominated-children behind -- a
    // stale entry makes dominator_preorder visit a block twice.
    for &id in &block_ids {
        if let Some(block) = cfg.get_block_mut(id) {
...
///
/// Natural loops sharing a header are merged into one loop (the standard
/// construction): a `continue` adds a second back-edge to the same `while`
/// header, and analyzing the two partial bodies separately hands consumers a
/// loop that is missing blocks -- LICM saw only the `continue` path, missed
/// the other back-edge source and the defs on the sibling branch, and hoisted
/// a conditional def (found by the Phase 4 property harness).
///
/// Returns: Vec of (header_id, loop_blocks), one entry per header, in
/// first-encounter order (deterministic for cache reproducibility).
    ...
    for edge in &cfg.edges {
        if let Some(source_block) = cfg.blocks.get(&edge.source) {
            if source_block.dominators.contains(&edge.target) {
                ...
                let loop_blocks = match loops.iter_mut().find(|(h, _)| *h == header) {
                    Some((_, blocks)) => blocks,
                    None => {
                        loops.push((header, HashSet::from([header])));
                        &mut loops.last_mut().unwrap().1
                    }
                };

                // BFS from source back to header (blocks in loop)
                let mut queue = VecDeque::new();
                if loop_blocks.insert(edge.source) {
