// FILE: catnip_core/src/cfg/ssa.rs
...
/// Operand in a value-numbering expression key: a variable (by SSA value) or a
/// scalar literal. Built by `ssa_cse::rhs_operands` and consumed by GVN, which
/// maps each operand to a value number.
...
impl SSAContext {
    ...
    /// Resolve a value through trivial-phi removal.
    ///
    /// The builder eliminates a phi whose incoming operands all agree (Braun et
    /// al. 2013, `tryRemoveTrivialPhi`) but leaves the value in `value_defs`,
    /// where `instruction_uses` may still reference it. Such a block parameter
    /// stands for its variable's current def at that block; this follows the
    /// chain to a fixpoint (bounded), so callers see the surviving definition.
    /// Used by SSA destruction (consistent naming) and LICM (a use of a trivial
    /// header phi is invariant if its resolution is defined outside the loop).
    pub fn resolve_trivial(&self, v: SSAValue) -> SSAValue {
        let mut cur = v;
        for _ in 0..64 {
            let block = match self.value_defs.get(&cur) {
                Some(ValueDef::BlockParam { block, .. }) => *block,
                _ => break,
            };
            match self.blocks.get(&block).and_then(|b| b.current_defs.get(&cur.var)) {
                Some(&repl) if repl != cur => cur = repl,
                _ => break,
            }
        }
        cur
    ...
    /// Verify SSA invariants that must hold after `SSABuilder::build`.
    ///
    /// Checks: every `ValueDef::BlockParam` points back to the param that
    /// actually defines it, sealed blocks carry no leftover incomplete phis,
    /// and each live block parameter has exactly one incoming operand per CFG
    /// predecessor (a phi with the wrong arity would read undefined values
    /// during destruction).
    ///
    /// Must be called before any CFG-mutating pass (LICM/IV split edges), which
    /// would change predecessor counts out from under the phi operands.
    pub fn verify(&self, cfg: &ControlFlowGraph) -> Result<(), String> {
        // BlockParam definitions must round-trip to the owning param.
        for (value, def) in &self.value_defs {
            if let ValueDef::BlockParam { block, param_idx } = def {
                let info = self
                    .blocks
                    .get(block)
                    .ok_or_else(|| format!("SSA: value defs reference unknown block {}", block))?;
                let param = info
                    .params
                    .get(*param_idx)
                    .ok_or_else(|| format!("SSA: block {} has no param at index {}", block, param_idx))?;
                if param.value != *value {
                    return Err(format!(
                        "SSA: block {} param {} defines {:?} but value_defs maps {:?} to it",
                        block, param_idx, param.value, value
                    ));
                }
            }
        }

        for (&block_id, info) in &self.blocks {
            // A sealed block must have resolved all its incomplete phis.
            if info.sealed && !info.incomplete_phis.is_empty() {
                return Err(format!("SSA: sealed block {} still has incomplete phis", block_id));
            }

            // Live phis must have one incoming operand per predecessor.
            let pred_count = SSAContext::get_predecessor_blocks(cfg, block_id).len();
            for param in &info.params {
                let is_live = info.current_defs.get(&param.value.var) == Some(&param.value);
                if is_live && param.incoming.len() != pred_count {
                    return Err(format!(
                        "SSA: block {} live phi for var {} has {} incoming operands but {} predecessors",
                        block_id,
                        param.value.var,
                        param.incoming.len(),
                        pred_count
...
mod tests {
    ...
    /// Build a diamond with x redefined in both branches → a live phi at merge.
    fn diamond_with_merge_phi() -> (ControlFlowGraph, SSAContext) {
        let cfg = diamond_cfg();
        let mut ssa = SSAContext::new();
        let (entry, cond, true_bb, false_bb, merge, exit) = (0, 1, 2, 3, 4, 5);

        ssa.seal_block(&cfg, entry);
        ssa.seal_block(&cfg, cond);
        ssa.seal_block(&cfg, true_bb);
        ssa.seal_block(&cfg, false_bb);
        ssa.def_var(entry, "x", 0);
        ssa.def_var(true_bb, "x", 0);
        ssa.def_var(false_bb, "x", 0);
        ssa.seal_block(&cfg, merge);
        ssa.seal_block(&cfg, exit);
        ssa.use_var(&cfg, merge, "x"); // creates the merge phi

        (cfg, ssa)
    }

    #[test]
    fn test_verify_ok_after_build() {
        let (cfg, ssa) = diamond_with_merge_phi();
        assert!(ssa.phi_count() > 0, "expected a live phi at merge");
        assert!(ssa.verify(&cfg).is_ok());
    }

    #[test]
    fn test_verify_detects_bad_phi_arity() {
        let (cfg, mut ssa) = diamond_with_merge_phi();
        let merge = 4;
        // Drop one incoming operand from the merge phi: arity no longer matches
        // the predecessor count.
        let info = ssa.blocks.get_mut(&merge).unwrap();
        info.params.first_mut().unwrap().incoming.pop();
        assert!(ssa.verify(&cfg).is_err());
    }

    #[test]
    fn test_verify_detects_unsealed_incomplete_phi() {
        let cfg = loop_cfg();
        let mut ssa = SSAContext::new();
        let (entry, header) = (0, 1);
        ssa.seal_block(&cfg, entry);
        ssa.def_var(entry, "x", 0);
        // Use x at the not-yet-sealed header → incomplete phi recorded.
        ssa.use_var(&cfg, header, "x");
        // Force the sealed flag without resolving the incomplete phi.
        ssa.blocks.get_mut(&header).unwrap().sealed = true;
        assert!(ssa.verify(&cfg).is_err());
    }
}
