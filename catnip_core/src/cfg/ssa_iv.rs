// FILE: catnip_core/src/cfg/ssa_iv.rs
...
fn name_of(ir: &IR) -> Option<String> {
    match ir {
...
/// Build a SetLocals target tuple holding a single variable name.
fn name_target(var_name: &str) -> IR {
    IR::Tuple(vec![IR::Ref(var_name.to_string(), -1, -1)])
}
...
fn extract_biv_step(op: &IR, var_name: &str) -> Option<i64> {
    let IR::Op { args, .. } = op else {
        return None;
    };
    // args[1] = rhs Op (Add or Sub)
    let rhs = args.get(1)?;
    let IR::Op { args: rhs_args, .. } = rhs else {
        return None;
    };
    if rhs_args.len() != 2 {
        return None;
    ...
    let left = &rhs_args[0];
    let right = &rhs_args[1];

    // Pattern: Ref(var) op constant  OR  constant op Ref(var)
    if let (IR::Ref(r, ..), IR::Int(c)) = (left, right) {
        if r == var_name {
            return Some(*c);
        }
    }
    if let (IR::Int(c), IR::Ref(r, ..)) = (left, right) {
        if r == var_name {
            return Some(*c);
        }
    }
...
fn extract_init_value(cfg: &ControlFlowGraph, ssa: &SSAContext, init_ssa: SSAValue) -> IVInit {
    let var_fallback = || IVInit::Variable(ssa.vars.name(init_ssa.var).unwrap_or("?").to_string());
    ...
    match args.get(1) {
        Some(IR::Int(val)) => IVInit::Constant(*val),
        _ => var_fallback(),
    }
}
...
fn extract_set_locals_name(op: &IR) -> Option<String> {
    if node_opcode(op) != Some(IROpCode::SetLocals) {
        return None;
    }
    let IR::Op { args, .. } = op else {
    ...
    match args.first() {
        Some(IR::Tuple(items)) if items.len() == 1 => name_of(&items[0]),
        Some(IR::Tuple(_)) => None,
        Some(other) => name_of(other),
        None => None,
    }
}
...
fn extract_div_info(op: &IR, biv_var: &str) -> Option<(String, i64)> {
    let IR::Op { args, .. } = op else {
        return None;
    };
    ...
    let var_name = match args.first() {
        Some(IR::Tuple(items)) if items.len() == 1 => name_of(&items[0])?,
        Some(IR::Tuple(_)) => return None, // Multi-assignment, skip
        Some(other) => name_of(other)?,
        None => return None,
    };
    ...
    for div in &result.divs {
        ...
        let init_value = match &biv.init {
            ...
            IVInit::Variable(_) => {
                ...
                let init_op = create_set_locals_mul(&div.var_name, &biv.var_name, div.scale);
                ...
                nop_and_insert_update(cfg, div, effective_step);
                continue;
            }
        };

        // Insert SetLocals(("j",), literal_init)
        let init_op = create_set_locals_literal(&div.var_name, init_value);
        if let Some(ph_block) = cfg.get_block_mut(preheader) {
            let pos = find_insert_position(ph_block);
            ph_block.instructions.insert(pos, init_op);
        }
...
fn nop_and_insert_update(cfg: &mut ControlFlowGraph, div: &DerivedIV, effective_step: i64) {
    // Nop the original Mul instruction
    if let Some(block) = cfg.get_block_mut(div.block) {
        if div.instr_idx < block.instructions.len() {
            if let IR::Op { opcode, .. } = &mut block.instructions[div.instr_idx] {
                *opcode = IROpCode::Nop;
            }
        }
    }
    ...
    let update_op = create_set_locals_add(&div.var_name, effective_step);
    if let Some(block) = cfg.get_block_mut(div.block) {
...
fn find_insert_position(block: &crate::cfg::basic_block::BasicBlock) -> usize {
    if let Some(last) = block.instructions.last() {
        let is_terminator = matches!(
            node_opcode(last),
            Some(IROpCode::OpReturn) | Some(IROpCode::OpBreak) | Some(IROpCode::OpContinue)
        );
        if is_terminator {
...
fn create_set_locals_literal(var_name: &str, value: i64) -> IR {
    IR::op(IROpCode::SetLocals, vec![name_target(var_name), IR::Int(value)])
}

/// Create SetLocals(("var",), Add(Ref("var"), step))
fn create_set_locals_add(var_name: &str, step: i64) -> IR {
    let add_op = IR::op(
        IROpCode::Add,
        vec![IR::Ref(var_name.to_string(), -1, -1), IR::Int(step)],
    );
    IR::op(IROpCode::SetLocals, vec![name_target(var_name), add_op])
}

/// Create SetLocals(("var",), Mul(Ref("src_var"), scale))
fn create_set_locals_mul(var_name: &str, src_var: &str, scale: i64) -> IR {
    let mul_op = IR::op(
        IROpCode::Mul,
        vec![IR::Ref(src_var.to_string(), -1, -1), IR::Int(scale)],
    );
    IR::op(IROpCode::SetLocals, vec![name_target(var_name), mul_op])
}
