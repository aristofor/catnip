// FILE: catnip_core/src/jit/codegen.rs
...
use super::trace_cache::NativeCodeCache;
...
fn build_cfg_blocks(trace: &Trace, ip_to_op: &HashMap<usize, usize>) -> Result<Vec<CfgBlock>, String> {
...
fn build_op_to_block(blocks: &[CfgBlock], op_count: usize) -> Vec<usize> {
    ...
    for (block_idx, block) in blocks.iter().enumerate() {
        for entry in op_to_block.iter_mut().take(block.end + 1).skip(block.start) {
            *entry = block_idx;
        }
    }
    op_to_block
...
fn compile_simple_op(
    ...
    match op {
        ...
        TraceOp::LeFloat => {
            ...
            let cmp = builder
                .ins()
                .fcmp(cranelift_codegen::ir::condcodes::FloatCC::LessThanOrEqual, a, b);
            stack.push(builder.ins().uextend(types::I64, cmp));
        }
        TraceOp::GtFloat => {
            let b = stack.pop().ok_or("Stack underflow")?;
        ...
        TraceOp::GeFloat => {
            ...
            let cmp = builder
                .ins()
                .fcmp(cranelift_codegen::ir::condcodes::FloatCC::GreaterThanOrEqual, a, b);
            stack.push(builder.ins().uextend(types::I64, cmp));
        }
        TraceOp::EqFloat => {
            let b = stack.pop().ok_or("Stack underflow")?;
...
impl JITCodegen {
    ...
    pub fn new() -> Result<Self, String> {
        ...
        let isa_builder = cranelift_native::builder().map_err(|e| format!("Failed to create ISA builder: {}", e))?;
        ...
        let native_cache = NativeCodeCache::new(get_cache_dir(), true);
        native_cache.cleanup_stale();
    ...
    pub fn compile(&mut self, trace: &Trace) -> Result<CompiledFn, String> {
        ...
        // Compile with incremental cache (loads stencil from disk on hit)
        let mut ctrl_plane = cranelift_codegen::control::ControlPlane::default();
        self.ctx
            .compile_with_cache(self.module.isa(), &mut self.native_cache, &mut ctrl_plane)
            .map_err(|e| format!("Failed to compile function: {:?}", e))?;

        // Extract compiled bytes + relocations for define_function_bytes
        let compiled_code = self
            .ctx
            .compiled_code()
            .ok_or_else(|| "No compiled code after compile_with_cache".to_string())?;
        let alignment = compiled_code.buffer.alignment as u64;
        let code = compiled_code.code_buffer();
        let relocs: Vec<cranelift_module::ModuleReloc> = compiled_code
            .buffer
            .relocs()
            .iter()
            .map(|r| cranelift_module::ModuleReloc::from_mach_reloc(r, &self.ctx.func, func_id))
            .collect();
    ...
    fn build_function_body(&mut self, trace: &Trace, self_func_id: Option<FuncId>) -> Result<(), String> {
        ...
        let block_stack_heights: Vec<usize> = block_heights_opt.iter().map(|h| h.unwrap_or(0)).collect();

        // Declare unbox functions if trace contains CallSelf
        let has_call_self = trace.ops.iter().any(|op| matches!(op, TraceOp::CallSelf { .. }));
        ...
        let enable_memoization = trace.trace_type == TraceType::Function && has_call_self && trace.num_params == 1;
        ...
        let max_depth = builder.ins().iconst(types::I32, JIT_MAX_RECURSION_DEPTH as i64);
        let too_deep = builder.ins().icmp(IntCC::SignedGreaterThan, depth, max_depth);
        ...
        builder.ins().brif(too_deep, overflow_block, &[], normal_path, &[]);
        ...
        let locals_ptr_slot = builder.create_sized_stack_slot(cranelift_codegen::ir::StackSlotData::new(
        ...
        use crate::nanbox::{
            PAYLOAD_MASK_I64, QNAN_BASE_I64, SMALLINT_SIGN_BIT_I64, SMALLINT_SIGN_EXT_I64, TAG_SMALLINT_I64,
        };
        ...
        for &slot in &locals_order {
            ...
            let val = if ty == types::F64 {
                ...
                let sign_bit_mask = builder.ins().iconst(types::I64, SMALLINT_SIGN_BIT_I64);
                let sign_bit = builder.ins().band(payload, sign_bit_mask);
                let zero = builder.ins().iconst(types::I64, 0);
                let is_negative = builder
                    .ins()
                    .icmp(cranelift_codegen::ir::condcodes::IntCC::NotEqual, sign_bit, zero);

                // Sign extend if negative: payload | SMALLINT_SIGN_EXT
                let sign_ext_mask = builder.ins().iconst(types::I64, SMALLINT_SIGN_EXT_I64);
            ...
            for i in block.start..=block.end {
                ...
                match op {
                    ...
                    TraceOp::JumpIfFalse(target) | TraceOp::JumpIfTrue(target) => {
                        ...
                        let is_true = builder
                            .ins()
                            .icmp(cranelift_codegen::ir::condcodes::IntCC::NotEqual, cond, zero);
                        ...
                        builder
                            .ins()
                            .brif(is_true, true_block, &true_args, false_block, &false_args);
                        terminated = true;
                        break;
                    }
                    TraceOp::GuardTrue | TraceOp::GuardFalse => {
                        ...
                        let true_block = true_block.map(|b| cl_blocks[b]).unwrap_or(guard_fail_block);
                        let false_block = false_block.map(|b| cl_blocks[b]).unwrap_or(guard_fail_block);
                        ...
                        builder
                            .ins()
                            .brif(is_true, true_block, &block_args, false_block, &block_args);
                        terminated = true;
                        break;
                    }
                    TraceOp::LoopBack | TraceOp::Continue => {
                        ...
                        builder.ins().jump(cl_blocks[loop_header_block], &block_args);
                        terminated = true;
                        break;
                    }
                    TraceOp::Exit | TraceOp::Break => {
                    ...
                    TraceOp::CallBuiltinPure { builtin_id, num_args } => {
                        ...
                        let call_builtin_fn = self
                            .module
                            .declare_function("catnip_call_builtin", Linkage::Import, &call_builtin_sig)
                            .map_err(|e| format!("Failed to declare catnip_call_builtin: {}", e))?;
                        let call_builtin_callee = self.module.declare_func_in_func(call_builtin_fn, builder.func);

                        let na = *num_args as usize;

                        // Re-box arguments to NaN-boxed format for the callback
                        use crate::nanbox::{PAYLOAD_MASK_I64 as PAYLOAD_MASK_CB, QNAN_BASE_I64 as QNAN_BASE_CB};
                        ...
                        // Re-box each arg (always produce exactly 2 for the callback signature)
                        let zero = builder.ins().iconst(types::I64, 0);
                        let mut boxed_args = [zero, zero];
                        for (j, arg) in args.iter().enumerate().take(2) {
                            let payload_mask = builder.ins().iconst(types::I64, PAYLOAD_MASK_CB);
                            let payload = builder.ins().band(*arg, payload_mask);
                            let qnan = builder.ins().iconst(types::I64, QNAN_BASE_CB);
                            boxed_args[j] = builder.ins().bor(qnan, payload);
                        }
                        ...
                        let call_inst = builder
                            .ins()
                            .call(call_builtin_callee, &[bid, boxed_args[0], boxed_args[1], nargs_val]);
                        ...
                        builder.ins().brif(is_fail, cb_fail_block, &[], cb_ok_block, &[]);
                        ...
                        builder.ins().jump(guard_fail_block, &to_block_args(&fail_args));
                    ...
                    TraceOp::CallSelf { num_args } => {
                        ...
                        if let Some(callee) = self_callee {
                            ...
                            let after_memo_block = if let Some(memo_lookup_callee) =
                                memo_lookup_callee.filter(|_| *num_args == 1)
                            {
                                // Get first argument (unboxed) for memoization key
                                let first_arg = *stack.last().ok_or("Stack underflow in CallSelf")?;

                                // Call memo_lookup(func_id_hash, first_arg)
                                let func_id_hash_const = builder.ins().iconst(types::I64, func_id_hash as i64);
                                let lookup_inst =
                                    builder.ins().call(memo_lookup_callee, &[func_id_hash_const, first_arg]);
                                let lookup_results = builder.inst_results(lookup_inst);
                                let cached_result_boxed = lookup_results[0];

                                // Check if cache hit (result != -1)
                                let minus_one = builder.ins().iconst(types::I64, -1);
                                let is_cache_hit = builder.ins().icmp(IntCC::NotEqual, cached_result_boxed, minus_one);
                                ...
                                builder
                                    .ins()
                                    .brif(is_cache_hit, cache_hit_block, &[], cache_miss_block, &[]);
                                ...
                                let cached_result = if let Some(unbox_fn) = unbox_float_callee {
                                    ...
                                    let unbox_inst = builder.ins().call(unbox_fn, &[cached_result_boxed]);
                                    ...
                                    return Err("Unbox function not available for memoization".into());
                                };
                            ...
                            let locals_array_slot = builder.create_sized_stack_slot(StackSlotData::new(
                            ...
                            use crate::nanbox::{
                                PAYLOAD_MASK_I64 as PAYLOAD_MASK_CALL, QNAN_BASE_I64 as QNAN_BASE_CALL,
                                TAG_SMALLINT_I64 as TAG_SMALLINT_CALL,
                            };
                            ...
                            for i in 0..*num_args {
                                ...
                                let boxed_arg = if arg_ty == types::F64 {
                                    ...
                                    let qnan_base = builder.ins().iconst(types::I64, QNAN_BASE_CALL);
                                    let tag_smallint = builder.ins().iconst(types::I64, TAG_SMALLINT_CALL);
                            ...
                            let locals_ptr = builder.ins().stack_addr(types::I64, locals_array_slot, 0);
                            ...
                            let call_inst = builder.ins().call(callee, &[locals_ptr, depth_incremented]);
                            let results = builder.inst_results(call_inst);
                            let boxed_result = results[0];

                            // Phase 4.3: Store result in memoization cache (before unboxing)
                            if let Some(memo_store_callee) = memo_store_callee.filter(|_| *num_args == 1) {
                                let first_arg = stack[stack.len() - num_args];
                                let func_id_hash_const = builder.ins().iconst(types::I64, func_id_hash as i64);
                                builder
                                    .ins()
                                    .call(memo_store_callee, &[func_id_hash_const, first_arg, boxed_result]);
                            }
                            ...
                            builder
                                .ins()
                                .brif(is_guard_fail, callself_guard_fail, &[], callself_continue, &[]);
                            ...
                            builder.ins().jump(guard_fail_block, &to_block_args(&guard_args));
                    ...
                    // SmallInt arithmetic with overflow deopt.
                    // Compute i64 result, check 47-bit range, bail to
                    // interpreter on overflow (which promotes to BigInt).
                    TraceOp::AddInt | TraceOp::SubInt | TraceOp::MulInt => {
                        let b = stack.pop().ok_or("Stack underflow")?;
                        let a = stack.pop().ok_or("Stack underflow")?;
                        let result = match op {
                            TraceOp::AddInt => builder.ins().iadd(a, b),
                            TraceOp::SubInt => builder.ins().isub(a, b),
                            TraceOp::MulInt => builder.ins().imul(a, b),
                            _ => unreachable!(),
                        };

                        // Range check: SMALLINT_MIN <= result <= SMALLINT_MAX
                        let max_val = builder.ins().iconst(types::I64, (1_i64 << 46) - 1);
                        let min_val = builder.ins().iconst(types::I64, -(1_i64 << 46));
                        let too_big = builder.ins().icmp(IntCC::SignedGreaterThan, result, max_val);
                        let too_small = builder.ins().icmp(IntCC::SignedLessThan, result, min_val);
                        let overflow = builder.ins().bor(too_big, too_small);

                        // Build guard_fail_block args: current locals + padding to exit_stack_height.
                        // On overflow the op hasn't committed (no StoreLocal yet), so locals are clean.
                        // The stack portion is padded with zeros since the VM will replay the op.
                        let zero = builder.ins().iconst(types::I64, 0);
                        ...
                        // Continuation block carries locals + stack (with result pushed)
                        stack.push(result);
                        ...
                        builder
                            .ins()
                            .brif(overflow, guard_fail_block, &fail_block_args, cont, &cont_block_args);
        ...
        for (&slot, &var) in &slot_vars {
            ...
            let val_i64 = if ty == types::F64 {
                ...
                let qnan_base = builder.ins().iconst(types::I64, QNAN_BASE_I64);
                let tag_smallint = builder.ins().iconst(types::I64, TAG_SMALLINT_I64);
        ...
        let ret_val = match trace.trace_type {
            ...
            TraceType::Function => {
                ...
                if !stack_values.is_empty() {
                    ...
                    builder.ins().iconst(types::I64, QNAN_BASE_I64)
                }
            }
        };
