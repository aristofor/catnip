// FILE: catnip_core/src/jit/trace_cache.rs
...
impl TraceCache {
    ...
    fn cache_key(&self, bytecode_hash: u64, loop_offset: usize) -> String {
        format!("jit_v{}_{:016x}_{:06x}", CACHE_VERSION, bytecode_hash, loop_offset)
    }
    ...
    pub fn store(&self, bytecode_hash: u64, loop_offset: usize, trace: &Trace) {
        ...
        if !self.directory.exists() && fs::create_dir_all(&self.directory).is_err() {
            return;
        }
    ...
    pub fn clear(&self) -> u64 {
        ...
        if let Ok(entries) = fs::read_dir(&self.directory) {
            for entry in entries.flatten() {
                let path = entry.path();
                let dominated = path.file_name().and_then(|n| n.to_str()).is_some_and(Self::is_jit_file);
                if path.is_file() && dominated && fs::remove_file(&path).is_ok() {
                    removed += 1;
                }
            }
        }
...
impl NativeCodeCache {
    ...
    fn file_path(&self, key: &[u8]) -> PathBuf {
        ...
        self.directory.join(format!("{}{hex}", self.prefix))
    }
    ...
    /// Remove stale native code files from previous versions.
    /// Called once at startup to clean up unversioned `jit_native_*` files
    /// and versioned files from older CACHE_VERSION.
    pub fn cleanup_stale(&self) {
        ...
        for entry in entries.flatten() {
            ...
            // Remove unversioned legacy files
            if name.starts_with("jit_native_") {
                let _ = fs::remove_file(entry.path());
            }
            // Remove versioned files from older versions
            if name.starts_with("jit_nv") && !name.starts_with(&self.prefix) {
                let _ = fs::remove_file(entry.path());
            }
        }
    }
