// FILE: catnip_core/src/vm/mro.rs
    ...
    F: Fn(&str) -> Option<Vec<String>>,
{
    ...
    loop {
        ...
        for seq in &sequences {
            let candidate = &seq[0];
            let in_tail = sequences.iter().any(|s| s.len() > 1 && s[1..].contains(candidate));
            if !in_tail {
        ...
        let head =
            found.ok_or_else(|| format!("Cannot create a consistent MRO for '{}': C3 linearization failed", name))?;
