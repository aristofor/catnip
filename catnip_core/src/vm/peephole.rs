// FILE: catnip_core/src/vm/peephole.rs
...
impl PeepholeOptimizer {
    ...
    pub fn optimize(instructions: Vec<Instruction>, line_table: Vec<u32>) -> (Vec<Instruction>, Vec<u32>) {
    ...
    fn resolve_jumps(mut instrs: Vec<Instruction>) -> Vec<Instruction> {
        ...
        for i in 0..instrs.len() {
            if instrs[i].op == VMOpCode::Jump {
                ...
                while target < instrs.len() && instrs[target].op == VMOpCode::Jump && seen.insert(target) {
        ...
        for (i, instr) in instrs.iter_mut().enumerate() {
            if instr.op == VMOpCode::Jump && instr.arg as usize == i + 1 {
                instr.op = VMOpCode::Nop;
            }
        }
    ...
    fn mark_live_code(instrs: &[Instruction]) -> HashSet<usize> {
        ...
        while let Some(idx) = to_visit.pop() {
            ...
            if matches!(
                instr.op,
                VMOpCode::Return | VMOpCode::Break | VMOpCode::Continue | VMOpCode::Halt | VMOpCode::Exit
            ) {
                continue;
            }
            ...
            if instr.op == VMOpCode::ForRangeStep {
                let jump_target = (instr.arg & super::FOR_RANGE_STEP_JUMP_MASK) as usize;
            ...
            if instr.op == VMOpCode::ForRangeInt {
                let jump_offset = (instr.arg & super::FOR_RANGE_JUMP_MASK) as usize;
