...
module.exports = grammar({
  ...
  rules: {
    ...
    struct_method: $ => seq(
      repeat($.decorator),
      choice(
        seq('op', field('op_symbol', $.operator_symbol)),
    ...
    _dict_entry: $ => choice($.dict_pair, $.dict_kwarg, $.dict_spread),
    ...
    number: $ => choice($.imaginary, $.decimal, $.float, $.integer),

    imaginary: $ => token(/\d+(\.\d+)?[jJ]/),

    decimal: $ => token(/\d+(\.\d+)?[dD]/),
