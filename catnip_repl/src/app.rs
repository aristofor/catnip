...
impl ExitReason {
    ...
    pub fn message(&self) -> &'static str {
        ...
        let nanos = SystemTime::now()
            ...
            .unwrap_or(0);
        // 1% weird
        let weird = constants::REPL_EXIT_WEIRD;
        if nanos % 100 == 0 {
            return weird[nanos / 100 % weird.len()];
        }
        let msgs = match self {
...
impl App {
    ...
    pub fn run(
        ...
        loop {
            ...
            self.viewport_y = cursor::position()?.1;
        ...
        match std::fs::read_to_string(filename) {
            Ok(code) => {
                ...
                for (i, line) in code.lines().enumerate() {
                    ...
                    match self.executor.execute(trimmed) {
                        Ok(result) => self.print_output(terminal, &result),
                        Err(e) => {
...
fn get_history_path(config: &ReplConfig) -> PathBuf {
    if let Some(home) = dirs_home() {
        home.join(&config.history_file)
    } else {
        PathBuf::from(&config.history_file)
    }
}
