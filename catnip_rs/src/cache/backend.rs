...
impl CatnipCache {
    ...
    fn get_parsed(
        ...
        let key = Py::new(
            py,
            CacheKey::new(source, CacheType::Ast, optimize, tco_enabled),
        )?;
    ...
    fn get_bytecode(
        ...
        let key = Py::new(
            py,
            CacheKey::new(source, CacheType::Bytecode, optimize, tco_enabled),
        )?;
        let backend = self.backend.bind(py);
        let result: Option<Py<CacheEntry>> = backend.call_method1("get", (key,))?.extract()?;
        ...
        for cache_type in cache_types {
            for optimize in [true, false] {
                for tco_enabled in [true, false] {
                    let key = Py::new(
                        py,
                        CacheKey::new(source.clone(), cache_type, optimize, tco_enabled),
                    )?;
                    let backend = self.backend.bind(py);
