...
#[test]
fn test_cfg_dominance_linear() {
    // Test dominance analysis on simple linear CFG
    Python::initialize();
    Python::attach(|py| {
        let assign_x = assign_node(py, "x", literal(py, 1));
        let assign_y = assign_node(py, "y", literal(py, 2));
        ...
        let builder = IRCFGBuilder::new("linear");
        let mut cfg = builder.build(vec![assign_x, assign_y]);

        // Use analysis module function
        super::analysis::compute_dominators(&mut cfg);

        let blocks: Vec<_> = cfg.get_reachable_blocks().into_iter().collect();
        assert_eq!(blocks.len(), 2, "Linear CFG has 2 blocks");

        let entry = blocks.iter().min().copied().unwrap();
        let exit = blocks.iter().max().copied().unwrap();

        // Check that exit block is dominated by entry
        if let Some(exit_block) = cfg.get_block(exit) {
            assert!(
                exit_block.dominators.contains(&entry),
