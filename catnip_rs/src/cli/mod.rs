...
/// Strip shebang line (e.g. `#!/usr/bin/env catnip`) from source code.
fn strip_shebang(code: String) -> String {
    if code.starts_with("#!") {
        match code.find('\n') {
            Some(pos) => code[pos + 1..].to_string(),
            None => String::new(),
        }
    } else {
        code
...
impl SourceInput {
    ...
    /// Load source from file path (strips shebang if present).
    pub fn from_file<P: AsRef<Path>>(path: P) -> io::Result<Self> {
        let path_str = path.as_ref().to_string_lossy().to_string();
        let content = strip_shebang(fs::read_to_string(&path)?);
        Ok(Self::File(path_str, content))
    }

    /// Load source from stdin (strips shebang if present).
    pub fn from_stdin() -> io::Result<Self> {
        let mut buffer = String::new();
        io::stdin().read_to_string(&mut buffer)?;
        Ok(Self::Stdin(strip_shebang(buffer)))
    }
...
mod tests {
    ...
    fn test_strip_shebang_with_shebang() {
        let code = "#!/usr/bin/env catnip\nx = 10\nx + 1".to_string();
        assert_eq!(strip_shebang(code), "x = 10\nx + 1");
    }
    ...
    fn test_strip_shebang_only_shebang() {
        let code = "#!/usr/bin/env catnip".to_string();
        assert_eq!(strip_shebang(code), "");
    }

    #[test]
    fn test_strip_shebang_with_args() {
        let code = "#!/usr/bin/env catnip -v\nx = 42".to_string();
        assert_eq!(strip_shebang(code), "x = 42");
    }

    #[test]
    fn test_strip_shebang_hash_not_bang() {
        let code = "# comment\nx = 10".to_string();
        assert_eq!(strip_shebang(code), "# comment\nx = 10");
    }
}
