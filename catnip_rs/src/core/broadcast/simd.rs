// FILE: catnip_rs/src/core/broadcast/simd.rs
...
fn div_int_to_float(data: &[i64], operand: i64) -> Option<Vec<f64>> {
    ...
    Some(data.iter().map(|&x| (x as f64) / (operand as f64)).collect())
}
...
pub fn try_simd_map(
    ...
    match data {
        NumericData::Ints(ref vals) => {
            ...
            if let Ok(operand_val) = operand.extract::<i64>() {
                // Arithmétique
        ...
        NumericData::Floats(ref vals) => {
            if let Ok(operand_val) = operand.extract::<f64>() {
                // Arithmétique
