...
//!
//! Creates Lambda instances with full TCO trampoline support.
...
impl Registry {
    ...
    pub(crate) fn op_call(
        ...
        let eval_kwargs = if let Some(ref node_obj) = node {
            ...
            node_obj.bind(py).getattr("kwargs").ok().map(|kwargs_dict| {
                ...
                if let Ok(items) = kwargs_dict.call_method0("items") {
                    for item in items.try_iter().unwrap().flatten() {
        ...
        if is_tail_call && tco_enabled {
            ...
            let tail_call = tail_call_class.call1((func, args_tuple.into_any(), kwargs_dict.into_any()))?;
    ...
    fn build_super_proxy(&self, py: Python<'_>, bm: &crate::core::BoundCatnipMethod) -> PyResult<Option<Py<PyAny>>> {
        // Get the real type's MRO from the instance
        let real_type_info = if let Ok(proxy_ref) = bm.instance.bind(py).cast::<crate::vm::CatnipStructProxy>() {
            let st_ref = proxy_ref.borrow().struct_type.as_ref().map(|s| s.clone_ref(py));
        ...
        let mut method_sources: std::collections::HashMap<String, String> = std::collections::HashMap::new();
    ...
    ///
    /// Creates a Lambda with full trampoline TCO support.
    /// The lambda captures the current closure scope at creation time.
    ///
    /// Takes a Bound reference to self to pass it to Lambda constructor.
    pub(crate) fn op_lambda(bound_self: &Bound<'_, Self>, args: &Bound<'_, PyTuple>) -> PyResult<Py<PyAny>> {
        ...
        // Create Lambda directly (100% Rust implementation)
        let registry_obj = bound_self.clone().into_any().unbind();
        let lambda = Lambda::new(py, body, registry_obj, Some(params))?;
