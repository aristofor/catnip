...
impl Registry {
    ...
    /// Build a SuperProxy for a bound method call, if the instance has parent methods.
    fn build_super_proxy(
        &self,
        py: Python<'_>,
        bm: &crate::core::BoundCatnipMethod,
    ) -> PyResult<Option<Py<PyAny>>> {
        if let Some(ref source_type) = bm.super_source_type {
            // Chain resolution: look up source type's CatnipStructType in globals
            let ctx = self.ctx.bind(py);
            let globals = ctx.getattr("globals")?;
            let cls = globals.call_method1("get", (source_type.as_str(), py.None()))?;
            if cls.is_none() {
                return Ok(None);
            }
            if let Ok(st_bound) = cls.cast::<crate::vm::CatnipStructType>() {
                let st = st_bound.borrow();
                if st.super_methods.is_empty() {
                    return Ok(None);
                }
                let methods: std::collections::HashMap<String, Py<PyAny>> = st
                    .super_methods
                    .iter()
                    .map(|(k, v)| (k.clone(), v.clone_ref(py)))
                    .collect();
                let parent_name = st.parent_type_name.clone().unwrap_or_default();
                drop(st);
                ...
                return Ok(Some(Py::new(py, proxy)?.into_any()));
            }
        } else if let Ok(proxy_ref) = bm.instance.bind(py).cast::<crate::vm::CatnipStructProxy>() {
            // Normal call: get super info from the instance's struct_type
            let st_ref = proxy_ref
                .borrow()
                .struct_type
                .as_ref()
                .map(|s| s.clone_ref(py));
            if let Some(st_py) = st_ref {
    ...
    fn maybe_call_post_init(&self, py: Python<'_>, result: Py<PyAny>) -> PyResult<Py<PyAny>> {
        ...
        let has_super = !st.super_methods.is_empty();
        let super_methods: std::collections::HashMap<String, Py<PyAny>> = st
            .super_methods
            .iter()
            .map(|(k, v)| (k.clone(), v.clone_ref(py)))
            .collect();
        let type_name = st.name.clone();
