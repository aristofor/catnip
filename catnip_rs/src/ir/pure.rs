...
impl IRPure {
    ...
    pub fn is_literal(&self) -> bool {
        matches!(
            self,
            IRPure::Int(_) | IRPure::Float(_) | IRPure::String(_) | IRPure::Bool(_) | IRPure::None
        )
    }
...
mod tests {
    ...
    fn test_compact_ref_and_identifier() {
        let v: serde_json::Value =
            serde_json::from_str(&IRPure::Ref("x".into()).to_compact_json()).unwrap();
        assert_eq!(v["ref"], "x");
