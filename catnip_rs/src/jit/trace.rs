// FILE: catnip_rs/src/jit/trace.rs
...
impl Trace {
    ...
    pub fn is_compilable(&self) -> bool {
        ...
        let fallback_count = self
            .ops
            .iter()
            .filter(|op| matches!(op, TraceOp::Fallback(_)))
            .count();
        fallback_count == 0
    }
...
impl TraceRecorder {
    ...
    pub fn record_opcode(&mut self, op: OpCode, arg: u32, is_int_value: bool, ip: usize) -> bool {
        ...
        let trace_op = match op {
            ...
            OpCode::ForRangeInt => {
                // Loop control: add loop condition guard
                // The trace should check if we should continue looping
                // arg encoding: slot_i, slot_stop, step_positive, jump_offset
                let slot_i = (arg >> 24) as usize;
                let slot_stop = ((arg >> 16) & 0xFF) as usize;
                let step_positive = ((arg >> 15) & 1) == 0;

                // Load i and stop, compare
            ...
            OpCode::ForRangeStep => {
                let slot_i = (arg >> 24) as usize;
                let step = ((arg >> 16) & 0xFF) as i8 as i64;
                if !trace.locals_used.contains(&slot_i) {
                    trace.locals_used.push(slot_i);
                }
                trace.ops.push(TraceOp::LoadLocal(slot_i));
...
mod tests {
    ...
    fn test_record_fallback() {
        ...
        assert!(matches!(
            trace.ops[0],
            TraceOp::Fallback(OpCode::LoadGlobal)
        ));
        assert!(!trace.is_int_only);
        assert!(!trace.is_compilable());
    }
