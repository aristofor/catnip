...
impl PyRustVM {
    ...
    fn execute(
        ...
        // Convert Python closure scope to native (if provided)
        let native_closure = closure_scope
            .as_ref()
            .and_then(|cs| vm::frame::py_scope_to_native(py, cs).ok());
    ...
    fn execute_simple(
        ...
        let result = self
            .vm
            .execute(py, std::sync::Arc::new(code), &[])
            .map_err(PyErr::from)?;
...
fn _rs(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_class::<policy::ModulePolicy>()?;
    ...
    // Register weird_log module
    weird_log::register_module(m)?;
