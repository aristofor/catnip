...
fn transform_number(node: Node, source: &str) -> TransformResult {
    ...
    if text.ends_with('j') || text.ends_with('J') {
        ...
        return Ok(IRPure::Imaginary(digits.to_string()));
    }

    // Decimal literal: suffix d/D
    if text.ends_with('d') || text.ends_with('D') {
        let digits = &text[..text.len() - 1];
        return Ok(IRPure::Decimal(digits.to_string()));
    }
            ...
            "interpolation" => {
                ...
                if let Some(expr_n) = expr_node {
                    ...
                    let value = if let Some(ref conv) = conversion {
                        ...
                        IRPure::Call {
                            func: Box::new(IRPure::Ref(func_name.to_string(), -1, -1)),
                            args: vec![expr],
                            kwargs: HashMap::new(),
                            start_byte: 0,
                            end_byte: 0,
                    ...
                    let result = if let Some(ref spec) = format_spec {
                        IRPure::Call {
                            func: Box::new(IRPure::Ref("format".to_string(), -1, -1)),
                        ...
                        IRPure::Call {
                            func: Box::new(IRPure::Ref("str".to_string(), -1, -1)),
                            args: vec![value],
                            kwargs: HashMap::new(),
                            start_byte: 0,
                            end_byte: 0,
...
fn transform_identifier(node: Node, source: &str) -> TransformResult {
    ...
    Ok(IRPure::Ref(text.to_string(), start_byte, end_byte))
}
...
fn build_assignment(lvalue: IRPure, rvalue: IRPure, start: usize, end: usize) -> IRPure {
    ...
    if let IRPure::Op { opcode, args, .. } = &lvalue {
        if *opcode == IROpCode::GetAttr {
            ...
            return IRPure::op_with_pos(IROpCode::SetAttr, setattr_args, start, end);
        }
        if *opcode == IROpCode::GetItem {
            let mut setitem_args = args.clone();
            setitem_args.push(rvalue);
            return IRPure::op_with_pos(IROpCode::SetItem, setitem_args, start, end);
        }
    }

    // SetLocals for identifiers and unpacking
    let (names_tuple, explicit_unpack) = match &lvalue {
        IRPure::Tuple(_) => (lvalue, IRPure::Bool(true)),
        _ => (IRPure::Tuple(vec![lvalue]), IRPure::Bool(false)),
    };
    IRPure::op_with_pos(
        IROpCode::SetLocals,
...
fn transform_assignment(node: Node, source: &str) -> TransformResult {
    ...
    if num_decorators > 0 {
        ...
        for i in (0..num_decorators).rev() {
            ...
            rvalue = IRPure::call(decorator, vec![rvalue]);
        }
    }
...
fn transform_list_literal(node: Node, source: &str) -> TransformResult {
        ...
        if child.kind() == "collection_items" {
            for item_child in named_children(&child) {
                ...
                if current.kind() == "collection_spread" {
                    ...
                    spread_entries.push(IRPure::op_with_pos(
                        IROpCode::TupleLiteral,
                        vec![IRPure::Bool(true), value],
                        current.start_byte(),
                        current.end_byte(),
                    ));
                    has_spread = true;
    ...
    if has_spread {
        return Ok(IRPure::Call {
            func: Box::new(IRPure::Ref("__catnip_spread_list".to_string(), -1, -1)),
            args: vec![IRPure::op_with_pos(
                IROpCode::TupleLiteral,
                spread_entries,
                node.start_byte(),
                node.end_byte(),
...
fn transform_tuple_literal(node: Node, source: &str) -> TransformResult {
    ...
    if has_spread {
        return Ok(IRPure::Call {
            func: Box::new(IRPure::Ref("__catnip_spread_tuple".to_string(), -1, -1)),
            args: vec![IRPure::op_with_pos(
                IROpCode::TupleLiteral,
                spread_entries,
                node.start_byte(),
...
fn transform_dict_literal(node: Node, source: &str) -> TransformResult {
    ...
    if has_spread {
        return Ok(IRPure::Call {
            func: Box::new(IRPure::Ref("__catnip_spread_dict".to_string(), -1, -1)),
            args: vec![IRPure::op_with_pos(
                IROpCode::TupleLiteral,
                spread_entries,
                node.start_byte(),
...
fn transform_set_literal(node: Node, source: &str) -> TransformResult {
    ...
    if has_spread {
        return Ok(IRPure::Call {
            func: Box::new(IRPure::Ref("__catnip_spread_set".to_string(), -1, -1)),
            args: vec![IRPure::op_with_pos(
                IROpCode::TupleLiteral,
                spread_entries,
                node.start_byte(),
...
fn transform_struct_stmt(node: Node, source: &str) -> TransformResult {
            ...
            "struct_method" => {
                ...
                if let Some(sym) = operator_symbol {
                    ...
                    method_name = Some(
                        operator_symbol_to_method_name(&sym, param_count)
                            .ok_or_else(|| {
                                format!("invalid operator '{}' with {} param(s)", sym, param_count)
                            })?
                            .to_string(),
                    );
                }
                ...
                if let Some(ref mname) = method_name {
                    ...
                    if !has_abstract && body.is_none() {
                        return Err(format!(
                            "method '{}' has no body (add @abstract or => {{...}})",
                            mname
                        )
                        .into());
                    }
                    ...
                    if has_static && !params.is_empty() {
                        if let IRPure::Tuple(ref pair) = params[0] {
                            if let Some(IRPure::String(ref pname)) = pair.first() {
                                if pname == "self" {
                                    return Err(format!(
                                        "@static method '{}' must not have 'self' parameter",
                                        mname
                                    )
                                    .into());
                                }
                ...
                if let Some(mname) = method_name {
                    let is_static_ir = IRPure::Bool(has_static);
                    if has_abstract {
    ...
    if !bases.is_empty() {
        args.push(IRPure::List(
            bases.into_iter().map(IRPure::String).collect(),
        ));
            ...
            "struct_method" => {
                ...
                if let Some(mname) = method_name {
                    ...
                    if has_abstract {
                        ...
                        let mbody = body.unwrap(); // safe: validated above
                        let lambda =
                            IRPure::op(IROpCode::OpLambda, vec![IRPure::List(params), mbody]);
                        methods.push(IRPure::Tuple(vec![
                            IRPure::String(mname),
                            lambda,
                            is_static_ir,
    ...
    for child in &children[1..] {
        match child.kind() {
            ...
            "callattr" => {
                ...
                let method =
                    IRPure::op(IROpCode::GetAttr, vec![result, IRPure::String(method_name)]);
...
fn apply_broadcast(target: IRPure, node: Node, source: &str) -> TransformResult {
    ...
    match final_kind {
        ...
        "broadcast" => {
            ...
            let inner =
                apply_broadcast(IRPure::Ref(param.to_string(), -1, -1), final_node, source)?;
