...
impl Compiler {
    ...
    fn compile_set_locals(
        ...
        // Check for complex patterns (star, nested) - use compile_unpack_pattern
        if let Some(ref pattern) = names_pattern {
            if self.has_complex_pattern(pattern) && values.len() == 1 {
                ...
                // Compile value, then use pattern-aware unpacking
                self.compile_node(py, &values[0])?;
                self.compile_unpack_pattern(py, &unwrapped, true)?;
                return Ok(());
            }
        }
    ...
    fn compile_for(&mut self, py: Python<'_>, args: &Bound<'_, PyAny>) -> PyResult<()> {
        ...
        // Check for range() optimization (only for simple variables)
    ...
    fn compile_for_range(
        ...
        // Push block before allocating slots (consistent with compile_for)
        let slot_start = self.locals.len();
        self.emit(VMOpCode::PushBlock, slot_start as u32);
    ...
    fn compile_call_node(&mut self, py: Python<'_>, node: &Bound<'_, PyAny>) -> PyResult<()> {
        ...
        if !kwargs_dict.is_empty() {
            ...
            // No keyword arguments - check if tail call
            // Note: IRPure::Call doesn't have tail flag, so always use regular Call
    ...
    fn compile_struct(&mut self, py: Python<'_>, args: &Bound<'_, PyAny>) -> PyResult<()> {
        ...
        // IR structure: (name, fields, implements, base, methods)
        // args[0] = name
        // args[1] = fields
        // args[2] = implements (list, may be empty) - optional
        // args[3] = base (string or None) - optional
        ...
        // Check if we have implements, base, and/or methods
        if args_len > 3 {
            // args[2] is implements, args[3] is base
            ...
            if !arg3.is_none() {
                base_name = arg3.extract::<String>().ok();
            }
            if args_len > 4 {
            ...
            if let Ok(base) = arg2.extract::<String>() {
                ...
                // implements list (possibly empty)
        ...
        // Fields are tuples (name, default_or_None)
        ...
        // Store constant: (name, fields_info, num_defaults, implements, base_or_None, [methods])
        // When implements is non-empty, always include it (new format).
        // When empty + no base, keep legacy format for backward compat.
        ...
        let struct_info = if !implements_list.is_empty() {
            // New format with implements: (name, fields, num_defaults, implements, base_or_None, [methods])
            ...
            let base_py: Py<pyo3::PyAny> = match base_name {
                Some(ref b) => b.as_str().into_pyobject(py)?.into_any().unbind(),
                None => py.None(),
            };
            ...
            // Legacy format (no implements)
            match (base_name, methods_list) {
                (Some(base), Some(methods)) => PyTuple::new(
                    py,
                    &[
                        name_py,
                        fields_tuple.into_any(),
                        num_defaults_py,
                        base.into_pyobject(py)?.into_any().as_any().clone(),
                        methods.into_any(),
                    ],
                )?,
                (Some(base), None) => PyTuple::new(
                    py,
                    &[
                        name_py,
                        fields_tuple.into_any(),
                ...
                (None, None) => {
                    PyTuple::new(py, &[name_py, fields_tuple.into_any(), num_defaults_py])?
                }
            }
        };
    ...
    fn compile_match(&mut self, py: Python<'_>, args: &Bound<'_, PyAny>) -> PyResult<()> {
        ...
        for i in 0..cases_len {
            ...
            // Swap result and value, pop value
            self.emit(VMOpCode::RotTwo, 0);
            self.emit(VMOpCode::PopTop, 0);
        ...
        // No match: pop value, push None
        self.emit(VMOpCode::PopTop, 0);
        let none_idx = self.add_const_value(py, py.None());
        self.emit(VMOpCode::LoadConst, none_idx as u32);
