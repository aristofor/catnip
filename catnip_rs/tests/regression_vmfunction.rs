...
/// Régression: auto-capture de lambda ne doit jamais capturer INVALID
///
/// Bug historique: lors de MakeFunction, la closure capturait parfois des slots
/// non initialisés (canary INVALID) au lieu de la résolution scope correcte.
#[test]
fn test_lambda_self_capture_no_invalid_canary() {
    assert_output(
        "sum_to = (n, acc = 0) => { if n <= 0 { acc } else { sum_to(n - 1, acc + n) } }; sum_to(10)",
        "55",
    );
}
