// FILE: catnip_rs/tests/run_control_flow.rs
//! Integration tests for catnip binary - Control flow.
...
fn test_while_loop() {
    ...
    assert_output("sum = 0; i = 1; while i <= 10 { sum = sum + i; i = i + 1 }; sum", "55");
}

#[test]
fn test_for_loop() {
    assert_output("sum = 0; for x in list(1, 2, 3) { sum = sum + x }; sum", "6");
    assert_output("result = 0; for i in range(5) { result = i }; result", "4");
}

#[test]
fn test_break() {
    assert_output("i = 0; while i < 10 { if i == 5 { break }; i = i + 1 }; i", "5");
}
