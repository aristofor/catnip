...
fn apply_formatting_rules(tokens: &[Token], config: &FormatConfig) -> String {
    ...
    for (i, token) in tokens.iter().enumerate() {
        ...
        if token.type_ == "LBRACE" {
            ...
            if needs_space_before(token, unary_context) && !result.is_empty() {
                if let Some(last) = result.last() {
                    if !last.ends_with(&[' ', '\n'][..]) {
                        result.push(" ".to_string());
                    }
            ...
            if needs_space_after(token, unary_context, next_token) {
                result.push(" ".to_string());
            }
        }
...
fn needs_space_before(token: &Token, prev_token: Option<&Token>) -> bool {
    let Some(prev) = prev_token else {
        return false;
    };
...
fn wrap_line(line: &str, config: &FormatConfig) -> String {
    ...
    let Some((break_pos, _)) = best else {
        return line.to_string();
    };
