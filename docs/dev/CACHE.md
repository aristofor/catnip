...
Système séparé du cache de compilation, dédié aux traces JIT.

**Module** : `catnip_rs/src/jit/trace_cache.rs`

**Emplacement** : `~/.cache/catnip/jit/`

**Sérialisation** : bincode (compact, rapide)

Ce cache persiste les traces enregistrées par le JIT pour éliminer le warm-up (100+ itérations) au prochain lancement. La clé combine un hash FNV-1a du bytecode (instructions + constant pool) et l'offset de la boucle. L'invalidation se fait par version Catnip et version de format.
