# Architecture VM pour les Opérations ND

Ce document explique comment les opérations non-déterministes (@@, @>, @[]) passent dans la VM et le bytecode.

## Vue d'ensemble

Les opérations ND sont compilées en 3 opcodes VM dédiés qui délèguent l'exécution au Registry Rust. L'architecture garantit :
...
Les formes broadcast `data.[@@ lambda]` et `data.[@> f]` utilisent l'opcode `Broadcast` existant avec des flags spéciaux.
...
Les opérations ND sont couvertes par 58 tests :

- `tests/language/test_nd_recursion.py` : 20 tests (formes de base + broadcast)
- `tests/optimization/test_nd_memoization.py` : 15 tests (cache, modes)
- `tests/optimization/test_nd_batching.py` : 11 tests (batching, pragmas)
- `tests/serial/test_nd_concurrency.py` : 12 tests (thread/process, déterminisme)

Tous passent en modes AST et VM (100% coverage).

## Références

- `catnip_rs/src/vm/opcode.rs` : Définitions opcodes ND
- `catnip_rs/src/vm/compiler.rs` : Compilation IR → bytecode
- `catnip_rs/src/vm/core.rs` : Handlers VM
- `catnip_rs/src/core/registry/nd.rs` : Logique ND (AST + VM)
- `catnip/nd.py` : NDScheduler, NDTopos, NDRecur
- `docs/lang/ND_RECURSION.md` : Spécification langage ND

> Cette architecture unifie les modes AST et VM via une délégation au Registry.
> L'utilisateur ne voit aucune différence de comportement, seule la performance change.
>
> Le VM évite l'overhead de l'interprétation AST (~10-20x plus rapide) tout en
> conservant la flexibilité du NDScheduler pour les modes parallèles.
