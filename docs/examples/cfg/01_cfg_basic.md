...
flowchart TD
    entry --> while_header
    while_header -->|condition true| while_body
    while_body --> while_header
    while_header -->|condition false| while_exit
    while_exit --> exit
```
...
flowchart TD
    entry --> while_header
    while_header -->|true| if_check
    while_header -->|false| while_exit
    if_check -->|break| while_exit
    if_check -->|continue| while_header
    if_check -->|normal path| loop_body
    loop_body --> while_header
    while_exit --> exit
```
...
> Les blocs basiques sont comme des wagons de train : ils vont toujours dans le même sens, sauf quand ils bifurquent. À
> ce moment-là ce ne sont plus des wagons, ce sont des aiguillages. Mais on continue de les appeler wagons par cohérence
> administrative.
