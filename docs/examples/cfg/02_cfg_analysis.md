...
flowchart TD
    ...
    outer_header -->|false| outer_exit
    inner_header -->|true| inner_body
    inner_body --> inner_header
    inner_header -->|false| outer_body
...
flowchart TD
    entry --> while_header
    while_header -->|true| if_break
    while_header -->|false| while_exit
    if_break -->|x == 5| while_exit
    if_break -->|else| loop_body
...
> En dominance, un bloc domine un autre si tous les chemins vers ce dernier passent par le premier. C'est une loi
> topologique, pas un vote démocratique.
