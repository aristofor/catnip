...
Après optimisation d'un CFG (fusion de blocs, suppression de dead code), on peut reconstruire du code structuré en
analysant :
...
> La reconstruction n'est pas utilisée dans le pipeline normal de compilation. C'est une feature expérimentale pour
> l'analyse et le debugging de CFG optimisés.
...
> Le if avec break est intégré dans le corps du while reconstruit. La reconstruction préserve la sémantique mais peut
> réorganiser les structures imbriquées.
