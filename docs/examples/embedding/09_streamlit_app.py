...
count""",
    ...
    # Options
    col1, col2 = st.sidebar.checkbox("Afficher variables", value=False), st.sidebar.checkbox(
        "Verbose", value=False
    )
    show_vars = col1
    verbose = col2
        ...
        if code.strip():
            try:
                ...
                if show_vars:
                    ...
                    variables = {
                        k: v
                        for k, v in catnip.context.globals.items()
...
def catnip_calculator():
    ...
    col1, col2, col3 = st.sidebar.selectbox("a", list(range(0, 101)), index=10), st.sidebar.selectbox(
        "b", list(range(0, 101)), index=20
