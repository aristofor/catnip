...
1. **Prototypage rapide**
   ...
   - POC avant intégration DSL

### ✗ Quand NE PAS utiliser standalone

- **Applications complètes** → Utilisez Python
- **Scripts > 200 lignes** → Utilisez Python avec Catnip DSL
- **Intégration avec libs complexes** → Python + Catnip DSL
- **Workflows multi-utilisateurs** → DSL dans application web
- **Règles métier modifiables** → DSL avec scripts en DB
...
**Note** : Pour manipulation CSV réelle, utilisez Python + pandas en mode DSL.
...
**Alternative DSL** : Voir `docs/examples/embedding/05_report_builder.py` pour rapports avec templates.
...
## Comparaison Standalone vs DSL

| Critère          | Standalone      | DSL                |
| ---------------- | --------------- | ------------------ |
...
→ **Utilisez Python** avec Catnip DSL pour les parties configurables.

### Exemple de migration standalone → DSL
