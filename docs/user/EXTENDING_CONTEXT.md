# Ajouter des fonctions au context

## Introduction

Catnip est volontairement **minimal côté I/O** : pas d'accès réseau/fichiers ni d'API haut niveau par défaut. `print` et
`write` servent juste la REPL ou l'exécution en console. L'hôte expose ensuite ce dont l'application a besoin, au bon niveau de contrôle.
