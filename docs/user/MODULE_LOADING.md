...
flowchart TD
    ...
    D --> E{"Trouvé ?"}
    E -->|Oui| F["Charger module local (.cat, .py, .so, package)"]
...
ext = import("myext", protocol="rs")       # cherche extension native (.so/.pyd) uniquement
```
...
Un module chargé une fois est mis en cache par son nom. Les appels suivants retournent le même objet :
