# Guide Shebang et Scripts Exécutables
...
# Catnip ne supporte pas `sys.argv` dans ce mode
# Utilisez un wrapper (shell/Python) pour passer des arguments

input_file = 'data.json'
print('Processing ' + input_file)
```

**Note** : Catnip ne supporte pas encore `sys.argv`. Pour scripts avec arguments, utilisez Python wrapper :

```bash
#!/bin/bash
# Wrapper shell pour passer arguments
catnip script.cat "$@"
```
...
Catnip ne supporte pas encore les arguments de ligne de commande.

**Workaround** : Wrapper shell ou Python

```bash
#!/bin/bash
# wrapper.sh - Passer arguments via env vars
export INPUT_FILE="$1"
catnip script.cat
```
...
```python
# script.cat
input_file = 'INPUT_FILE'  # Injecté via wrapper (env var)
```
