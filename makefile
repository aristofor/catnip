...
VENV_BIN     := .venv/bin
CIBW_CACHE_DIR ?= $(CURDIR)/.cache/cibuildwheel
...
build-deps:
	@python3 -c "import setuptools" 2>/dev/null || \
		uv pip install setuptools setuptools-rust wheel

venv:
	uv venv --prompt Catnip .venv
	@echo ""
	@echo "Next: make compile && make install"
...
setup:
	...
	uv pip install -U pytest setuptools setuptools_rust
	$(MAKE) install
	@echo ""
	@echo "Setup complete!"
...
# Install Python package (depends on extension)
...
install-mcp: install-lang
	@echo "$$_MCP_REGISTER" | $(VENV_PYTHON) - .mcp.json catnip $(CURDIR)/.venv/bin/python catnip_mcp.server
	@test -f "$(CLAUDE_GLOBAL_CFG)" && \
		echo "$$_MCP_REGISTER" | $(VENV_PYTHON) - "$(CLAUDE_GLOBAL_CFG)" catnip $(CURDIR)/.venv/bin/python catnip_mcp.server || true
...
# Quality & Tools
#══════════════════════════════════════════════════════════════════════════════
.PHONY: format check proof proof-clean proof-scan

format:
	cd catnip_rs && cargo fmt --all
	uvx black $(PY_SOURCES) catnip_mcp/ tests/
...
dist: clean-dist
	mkdir -p $(CIBW_CACHE_DIR)
	CIBW_CACHE_DIR=$(CIBW_CACHE_DIR) uvx cibuildwheel --output-dir dist
