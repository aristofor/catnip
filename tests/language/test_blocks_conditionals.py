...
class TestConditionals:
    ...
    def test_if_non_boolean_truthy_literal(self):
        """If with non-boolean truthy literals (int, str)"""
        cat = Catnip()
        cat.parse('if 1 { "a" } else { "b" }')
        assert cat.execute() == 'a'

        cat = Catnip()
        cat.parse('if 42 { "a" } else { "b" }')
        assert cat.execute() == 'a'

        cat = Catnip()
        cat.parse('if "hello" { "a" } else { "b" }')
        assert cat.execute() == 'a'

    def test_if_non_boolean_falsy_literal(self):
        """If with non-boolean falsy literals (0, None, empty string)"""
        cat = Catnip()
        cat.parse('if 0 { "a" } else { "b" }')
        assert cat.execute() == 'b'

        cat = Catnip()
        cat.parse('if None { "a" } else { "b" }')
        assert cat.execute() == 'b'

        cat = Catnip()
        cat.parse('if "" { "a" } else { "b" }')
        assert cat.execute() == 'b'

    def test_if_non_boolean_literal_with_cfg_optimization(self):
        """If with non-boolean literals under CFG optimizations (optimize_level=3)"""
        cat = Catnip()
