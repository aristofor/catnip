...
class TestCatnipInternalError:
    """Tests de CatnipInternalError."""

    def test_message_format(self):
        """Message d'erreur interne."""
        err = CatnipInternalError("Unexpected state in interpreter")
        assert "Unexpected state" in str(err)

    def test_scope_pop_error(self):
        """Tentative de pop du scope global."""
        catnip = Catnip()
        with pytest.raises(CatnipInternalError) as exc_info:
