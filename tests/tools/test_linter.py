...
class TestSemantic:
    ...
    def test_used_variable_no_warning(self):
        result = lint_code("x = 1\nprint(x)", check_syntax=False, check_style=False)
    ...
    def test_for_loop_var_defined(self):
        result = lint_code("for i in range(10) { print(i) }", check_syntax=False, check_style=False)
        errors = [d for d in result.diagnostics if d.code == "E300"]
        assert len(errors) == 0

    def test_match_capture_defined(self):
        result = lint_code("x = 1\nmatch x { n => { print(n) } }", check_syntax=False, check_style=False)
        errors = [d for d in result.diagnostics if d.code == "E300"]
        assert len(errors) == 0

    def test_builtins_predefined(self):
        result = lint_code("print(len(range(10)))", check_syntax=False, check_style=False)
        errors = [d for d in result.diagnostics if d.code == "E300"]
        assert len(errors) == 0

    def test_nested_scopes(self):
        """Variables from outer scope visible in inner scope."""
        result = lint_code("x = 1\nf = () => { print(x) }", check_syntax=False, check_style=False)
        errors = [d for d in result.diagnostics if d.code == "E300"]
        assert len(errors) == 0

    def test_inner_scope_not_leaking(self):
        """Variables from inner scope not visible in outer scope."""
        result = lint_code("f = () => { y = 1 }\nprint(y)", check_syntax=False, check_style=False)
        errors = [d for d in result.diagnostics if d.code == "E300"]
        assert len(errors) == 1
...
class TestIntegration:
    ...
    def test_lint_result_summary(self):
        result = lint_code("y = undefined_var")
